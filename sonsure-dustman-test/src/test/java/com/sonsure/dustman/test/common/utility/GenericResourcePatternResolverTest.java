package com.sonsure.dustman.test.common.utility;
...
public class GenericResourcePatternResolverTest {
    ...
    public void getClasspathFileResource() throws Exception {
        ...
        // 测试 classpath 资源加载
        List<GenericResource> classpathResources = resolver.getResources("classpath:/db/migration/mysql/V1.0.0__dustman_test.sql");
        Assertions.assertEquals(1, classpathResources.size());
        Assertions.assertEquals("V1.0.0__dustman_test.sql", classpathResources.get(0).getFilename());
    ...
    public void getClasspathAllFileResource() throws IOException {
        ...
        List<String> list = classpathResources.stream()
                .map(GenericResource::getFilename)
                .collect(Collectors.toList());
        Assertions.assertTrue(list.contains("V1.0.0__dustman_test.sql"));
        ...
        List<String> list2 = classpathResources.stream()
                .map(GenericResource::getFilename)
                .collect(Collectors.toList());
        Assertions.assertTrue(list2.contains("V1.0.0__dustman_test.sql"));
    ...
    public void getFileSystemResource() {
        ...
        List<GenericResource> resources = resolver.getResources(prefix + "/src/test/resources/db/migration/mysql/V1.0.0__dustman_test.sql");
        Assertions.assertEquals(1, resources.size());
        Assertions.assertEquals("V1.0.0__dustman_test.sql", resources.get(0).getFilename());

        resources = resolver.getResources(prefix + "/src/test/resources/*.xml");
        Assertions.assertEquals(1, resources.size());

        resources = resolver.getResources(prefix + "/src/test/resources/**.xml");
        Assertions.assertEquals(14, resources.size());
