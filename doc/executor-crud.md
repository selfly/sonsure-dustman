...
insert操作比较简单，没有什么复杂的where条件等设置。
    ...
    jdbcDao.insert() //or insertInto(UserInfo.class)
        .into(UserInfo.class)
        .intoField("loginName", "selfly")
        .intoField("password", "2019")
        .intoField("userAge", 18)
        .execute();
...
支持多种 JOIN 方式，实体类名和表名均可：

    // 实体类方式，自动转换表名
    List<UserInfo> list = jdbcDao.selectFrom(UserInfo.class).as("t1")
            .leftJoin(UserExtend.class).as("t2")
            .on(UserInfo::getUserInfoId, UserExtend::getUserId)
            .findList();

    // 表名方式，不经过转换
    List<Map<String, Object>> list2 = jdbcDao.selectFrom(UserInfo.class).as("t1")
            .leftJoin("user_extend").as("t2")
            .on("t1.user_info_id = t2.user_id")
            .addAliasColumn("t1", "loginName")
            .addAliasColumn("t2", "extField")
            .findListForMap();

支持的 JOIN 类型：`join`、`innerJoin`、`leftJoin`、`rightJoin`、`outerJoin`。
...
条件、列、排序等都支持 Lambda 表达式，编译期即可检查属性名正确性：

    List<UserInfo> list = jdbcDao.selectFrom(UserInfo.class)
            .where(UserInfo::getUserAge, SqlOperator.GT, 5)
            .and(UserInfo::getLoginName, "name-10")
            .orderBy(UserInfo::getUserInfoId, OrderBy.ASC)
            .findList();

    jdbcDao.update(UserInfo.class)
            .set(UserInfo::getLoginName, "newname")
            .set(UserInfo::getPassword, "newpwd")
            .where(UserInfo::getUserInfoId, 10L)
            .execute();

    jdbcDao.insertInto(UserInfo.class)
            .intoField(UserInfo::getLoginName, "selfly")
            .intoField(UserInfo::getUserAge, 18)
            .execute();
...
- condition(String field, SqlOperator sqlOperator, Object value) 带操作符的condition

condition 与 where 的区别在于 condition 不添加 and/or 关键字，适合在括号或复杂条件下使用：

    List<UserInfo> list = jdbcDao.selectFrom(UserInfo.class)
            .where("userAge", SqlOperator.GT, 5)
            .and()
            .openParen()
            .condition("loginName", SqlOperator.LIKE, "%name%")
            .or()
            .condition("password", SqlOperator.LIKE, "%pwd%")
            .closeParen()
            .findList();
